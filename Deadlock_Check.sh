...
if [ -e $D_log ] ; then 
   ...
   if [ "$Deadlock_id" != "$log_Deadlock_id" ] ;then
     ...
     mail -s "Mysql Deadlock Issue $Today" -S "smtp-use-starttls=yes" -S "sntp-auth=login" -S "smtp=smtp.gmail.com:587"  -S "smtp-auth-user=rd@1-pay.co" -S "smtp-auth-password=mg66=FBh" -S "from=rd@1-pay.co" -S "ssl-verify=ignore" -S "nss-config-dir=/root/DB_data/.certs" $mail_group  < $D_temp
     cat $D_temp > $D_log
   else
